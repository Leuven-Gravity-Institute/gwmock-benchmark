...
breaking change straight to the next major. At `v0.4.4`, with one breaking commit in the history since
...
class TestZeroVerHoldsTheMajorBack:
    ...
    def test_a_proposed_major_bump_becomes_a_minor_bump(self, current: str, proposed: str, expected: str) -> None:
        """The live case: `v0.4.4` plus a breaking change gives `v0.5.0`, not `v1.0.0`.
    ...
    def test_a_proposal_that_is_not_a_major_bump_is_untouched(self, current: str, proposed: str) -> None:
        ...
        The first row is this repository's own non-breaking case, measured: at `v0.4.4` with no breaking
        commit in the history since that tag, git-cliff proposes `v0.5.0` and the guard hands it
        straight back.
        """
    ...
    def test_the_input_can_still_ask_for_the_major(self) -> None:
        ...
        assert choose_version(current="v0.4.4", proposed="v1.0.0", allow_major_bump=True) == "v1.0.0"
...
class TestTheShapesItRefuses:
    ...
    def test_an_unparsable_proposal_is_refused(self, proposed: str) -> None:
        ...
        with pytest.raises(ValueError, match="could not read"):
            choose_version(current="v0.4.4", proposed=proposed)
...
class TestTheFormatItReturns:
    ...
    def test_the_v_prefix_is_preserved(self) -> None:
        ...
        assert choose_version(current="v0.4.4", proposed="v1.0.0").startswith("v")
    ...
    def test_a_prefixless_current_is_accepted_and_the_proposal_s_shape_is_kept(self) -> None:
        ...
        assert choose_version(current="0.4.4", proposed="1.0.0") == "0.5.0"
...
class TestWhitespaceNeverReachesTheTag:
    ...
    def test_the_demoted_path_is_clean_too(self) -> None:
        ...
        assert choose_version(current="v0.4.4", proposed=" v1.0.0 ") == "v0.5.0"
