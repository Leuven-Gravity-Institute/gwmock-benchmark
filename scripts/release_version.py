...
has not: at `v0.4.4`, a single `feat!:`/`fix!:`/`BREAKING CHANGE:` commit makes its answer `v1.0.0` --
...
Run as a script it takes the two versions and prints the answer, which is what the workflow uses:
    ...
    python scripts/release_version.py --current v0.4.4 --proposed "$(git-cliff --bumped-version)"
